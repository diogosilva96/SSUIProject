...
public class AudioListManipulator implements Serializable { //se for preciso para passar o objeto no intent
    ...
    public AudioListManipulator(ArrayList<String> aList){
        audioList = aList;
    ...
    public void setCurrentAudio(String name){
        ...
        Log.e("AudioListManipulator","Value for:"+name+" found:" +valueFound + " currentPosition on List:" +currentPosition + " Path: "+audioList.get(currentPosition));
    }
    ...
    private void addToPlayedList(){
        ...
        if(playedList.size()>0) {
            for (int i = 0; i < playedList.size(); i++) {
                Log.e("AudioListManipulator", "size:"+playedList.size()+" currentPos:" + currentPosition + " playedList.get(" + i + ")=" + playedList.get(i));
    ...
    private void clearIfEndOfPlayedList(){
        if (playedList.size()==audioList.size()){
            Log.e("AudioListManipulator","playedList cleared!");
            playedList.clear();
        }
    }
